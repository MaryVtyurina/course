...
(|||)  a b = P { parse = \i -> let
  x = parse a i
  in case parse a i of
    ErrorResult _ -> parse b i
    otherwise -> x
  }
...
list1 ::
  Parser a
  -> Parser (List a)
list1 a = flbindParser a (\i ->
      flbindParser (list a) (\j ->
            valueParser (i :. j)))
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy f = flbindParser character (\i -> if f i
  then valueParser i
  else unexpectedCharParser i)
...
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser = foldRight (\x xs ->
        flbindParser x (\i ->
              flbindParser xs (\j ->
                    valueParser (i :. j)))) (valueParser Nil)
...
firstNameParser ::
  Parser Chars
firstNameParser = flbindParser upper (\i ->
    flbindParser (list lower) (\j ->
        valueParser (i :. j)
    ))
...
personParser = flbindParser ageParser (\age ->
    ...
    flbindParser firstNameParser (\firstName ->
    flbindParser spaces1 (\_ ->
    flbindParser surnameParser (\surname ->
    flbindParser spaces1 (\_ ->
    flbindParser smokerParser (\smoker ->
    flbindParser spaces1 (\_ ->
    flbindParser phoneParser (\phone ->
    valueParser Person {
    age = age, firstName = firstName, surname = surname, smoker = smoker, phone = phone
    })))))))))
